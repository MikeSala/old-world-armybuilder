...
export function StatsTable({ rows, dict }: Props) {
  return (
    <div className="overflow-x-auto print:overflow-visible">
      <table className="min-w-full divide-y divide-stone-400/20 text-xs print:divide-gray-300 print:text-[11px]">
        <thead className="text-stone-200/70 print:text-gray-600">
          <tr>
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.label} className="text-stone-200 print:text-gray-900">
              ...
              {ROSTER_STAT_FIELDS.map((field) => (
                <td
                  ...
                  className="px-2 py-2 text-center text-stone-100 print:text-gray-900 print:text-xs"
                >
